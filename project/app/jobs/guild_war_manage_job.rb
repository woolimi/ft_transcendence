class GuildWarManageJob < ApplicationJob
  ...
  def perform(match_id)
    ...
    if (war.guild_1_matches_unanswered == war.max_unanswered || war.guild_2_matches_unanswered == war.max_unanswered)
      ...
      winner.guild.total_score += war.wager * 2
      winner.guild.save!()
    end
    war.save!()
    ActionCable.server.broadcast("match_#{match_id}_channel", {end: true, unanswered: true, data: m.jbuild() })
